...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.396332;
    vars_upperbound[i] = 0.396332;
  }
  ...
  for (int xx=x_start+1; xx<y_start; xx++) {
    soln.push_back(solution.x[xx]);
  }
  for (int xx=y_start+1; xx<psi_start; xx++) {
    soln.push_back(solution.x[xx]);
  }
  return soln;
}
